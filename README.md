# Hi there, I'm Purva Prakash Kekan! 👋

<div align="center">
  
[![Portfolio](https://img.shields.io/badge/Portfolio-FF5722?style=for-the-badge&logo=todoist&logoColor=white)](https://purva-kekan.github.io/portfolio/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/purva-kekan)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:purvakekan@gmail.com)

</div>

## 🎯 About Me

**Data Analyst @ Ipserlab** | Passionate about transforming raw data into actionable insights that drive meaningful change

Hey there! I’m a Data Analyst who transforms messy data into insights that leaders can act on 30% to 45% more accurately. My work helps public health, healthcare and product teams make faster, cleaner and more confident decisions without the overwhelm. I specialize in simplifying complex datasets, building high clarity dashboards and unlocking the real story behind numbers so organizations can scale impact, efficiency and trust.


🔍 **What drives me:** Using data + behavioral science to address real-world equity challenges in healthcare and community outcomes

🚀 **Currently building:** [CAVO](https://github.com/purva-kekan) - an inclusive AI-driven travel platform

🌟 **Beyond analytics:** When I'm not diving into data, you'll find me dancing, designing or exploring creative ways to visualize insights

---

## 💼 Featured Projects

### 🏥 [Healthcare Analytics - Geospatial Analysis](https://github.com/purva-kekan/healthcare-analytics-geospatial-analysis)
**Boston 311 Syringe Requests Dashboard** | Geospatial + Predictive Modeling
- Built interactive dashboards combining geospatial analysis with predictive models
- Technologies: Python, ArcGIS, Power BI, Predictive Modeling

### 📚 [Atomic Habits NLP Concept Graph](https://github.com/purva-kekan/atomichabits-nlp-project)
**Semantic Network Analysis**
- Created NLP-powered concept graphs to extract key insights from behavioral science literature
- Technologies: Python, NLP, Semantic Networks, Network Analysis

### 🫁 [Respiratory Mortality Forecasting](https://github.com/purva-kekan/respiratory-mortality-system)
**FastAPI + Machine Learning Pipeline**
- Developed forecasting models to predict respiratory mortality trends
- Technologies: FastAPI, Machine Learning, Python, Time Series Analysis

---

## 🛠️ Technical Skills

### Languages & Tools
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=flat&logo=r&logoColor=white)

### Data Visualization & BI
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=flat&logo=powerbi&logoColor=black)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white)
![ArcGIS](https://img.shields.io/badge/ArcGIS-2C7AC3?style=flat&logo=arcgis&logoColor=white)

### Specializations
- 📊 **Analytics:** Predictive Modeling, Forecasting, Statistical Analysis
- 🗺️ **Geospatial:** Spatial Analysis, GIS, Location Intelligence
- 🤖 **NLP:** Text Analytics, Semantic Networks, Behavioral Analytics
- 💡 **Focus Areas:** Public Health, Mental Health, Equity-Driven Analytics

---

## 🎓 Education

**Master of Science in Analytics** | Northeastern University  
*GPA: 3.93/4.0*

---

## 📊 What I'm Working On

- 🔬 Exploring how data + behavioral science can drive better decision-making
- 🌍 Building inclusive AI solutions for accessible travel experiences
- 📈 Developing geospatial dashboards for community health insights
- 🧠 Applying NLP to mental health and public health challenges

---

## 🌱 Philosophy

> "Data storytelling isn't just about numbers—it's about uncovering narratives that can create real-world impact, especially in addressing equity challenges in healthcare and community outcomes."

---

## 📫 Let's Connect!

I'm always excited to collaborate on projects involving **data analytics, public health, behavioral science, or social impact**. Feel free to reach out!

- 💼 [Portfolio](https://purva-kekan.github.io/portfolio/)
- 💌 [Email](mailto:purvakekan@gmail.com)
- 🔗 [LinkedIn](https://linkedin.com/in/purva-kekan)
- 📍 Based in Boston, MA

---

<div align="center">

### ⚡ Fun Fact
I believe the best insights come from blending creativity with data whether that's through dance, design, or data visualization!

</div>
